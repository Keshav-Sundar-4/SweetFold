...
class GradientDebugger(Callback):
    """
    Inspect gradients after the backward pass to find parameters that were not
    used in the computation graph.

    This is useful for debugging DDP errors involving unused parameters.
    """
    ...
    def on_after_backward(self, trainer, pl_module):
        if trainer.global_rank == 0:
            ...
            if unused_params:
                ...
                print("These parameters were likely part of a conditionally skipped module:", flush=True)
...
def train(raw_config: str, args: list[str]) -> None:
    ...
    ----------
    raw_config:
        Path to the input YAML configuration.
    args:
        Hydra-style command-line overrides.
    """
    ...
    if cfg.pretrained and not cfg.resume:
        if cfg.load_confidence_from_trunk:
            ...
            print(
                f"Saving modified checkpoint to {file_path} created by broadcasting "
                f"trunk of {cfg.pretrained} to confidence module."
            )
    ...
    if wandb_cfg:
        ...
        def save_config_to_wandb() -> None:
            ...
            try:
                ...
                wandb_logger.experiment.config.update(resolved_config_dict)
            except Exception as e:
    ...
    if num_devices > 1:
        ...
        print(f"Using DDP strategy with find_unused_parameters={find_unused_ddp}")
        strategy = DDPStrategy(find_unused_parameters=find_unused_ddp)
    elif cfg.find_unused_parameters and num_devices <= 1:
        print(
            f"Warning: find_unused_parameters={cfg.find_unused_parameters} requested "
            f"but only {num_devices} device(s)."
        )
